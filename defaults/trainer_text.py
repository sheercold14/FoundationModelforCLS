...
class Trainer(BaseTrainer):
    ...
    def global_step(self, **kwargs):
        ...
        if self.additional_loss:     
            ...
            if self.text_branch:
                images, labels, text = kwargs['batch']
                if len(labels) == 2 and isinstance(labels, list):
                    ids    = labels[1]
                    labels = labels[0]
                labels = labels.to(self.device_id, non_blocking=True)
    ...
    def evaluate(self, dataloader=None, **kwargs):
        ...
        with torch.no_grad():
            if self.text_branch:
                for images, labels, text in iter_bar:
                    if len(labels) == 2 and isinstance(labels, list):
                        ids    = labels[1]
                        labels = labels[0]
                    labels = labels.to(self.device_id, non_blocking=True)
                    ...
                    if is_ddp(self.model):
                        ...
                        outputs, features = self.model(images, text, return_embedding=True)
            ...
            for batch in iter_bar:
                if self.text_branch:
                    ...
                    if is_ddp(self.model):
                        ...
                        _, feature = self.model(images, text, return_embedding=True)
