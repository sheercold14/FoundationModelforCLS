...
class DefaultWrapper:
    ...
    def instantiate(self):        
        ...
        transfusion_name = self.model_params.backbone_type
        run_check = mname.split('-')[-1].split('_')
        if len(run_check) == 2 and run_check[0] == 'run':
            transfusion_name = f"{transfusion_name}-run_{run_check[1]}"
        transfusion_dir = os.path.join(self.training_params.save_dir, "transfusion_inits")
        self.model_params.transfusion_checkpoint = os.path.join(transfusion_dir, transfusion_name)
        ...
        #print("Length of data loader ", len(self.dataloaders.trainloader))
...
class SAMWrapper(DefaultWrapper):
    ...
    def init_model(self):      
        ...
        self.foundation_params.img_channels = self.model_params.img_channels
        self.foundation_params.knn_nhood = self.model_params.knn_nhood
        ...
        # INIT Ymodel model
        model = Ymodel_SAM(self.parameters, foundation_model, main_model)   
        if ddp_is_on():
            model = DDP(model, device_ids=[self.device_id], find_unused_parameters=False)

        print_ddp(f"\033[94m\t{model}\033[0m")
        return model

class DINOWrapper(DefaultWrapper):
    def __init__(self, parameters,):
        super().__init__(parameters)       
...
class SEEMWrapper(DefaultWrapper):
    def __init__(self, parameters,):
        super().__init__(parameters)       
    ...
    def init_model(self):      
        ...
        # INIT Ymodel model
        model = Ymodel_SEEM(self.parameters, foundation_model, main_model)   
        if ddp_is_on():
            model = DDP(model, device_ids=[self.device_id], find_unused_parameters=False)
        return model

class CLIPWrapper(DefaultWrapper):
    def __init__(self, parameters,):
        super().__init__(parameters)       
    ...
    def init_model(self):      
        ...
        # INIT Ymodel model
        model = Ymodel_CLIP(self.parameters, foundation_model, main_model)   
        if ddp_is_on():
            model = DDP(model, device_ids=[self.device_id], find_unused_parameters=False)
        return model

class BLIPWrapper(DefaultWrapper):
    def __init__(self, parameters,):
        super().__init__(parameters)       
...
class FoundationWrapper(DefaultWrapper):
    def __init__(self, parameters):
        super().__init__(parameters)       
