...
def parse_arguments():
    ...
    parser.add_argument('--knn_eval', action='store_true', default=False, help='Flag for knn_eval')
    ...
    """cold for cross validation"""
    parser.add_argument('--cross_folder', type=str, help='folder num for cross validation')
...
def update_params_from_args(params, args):
    ...
    if args.cross_folder:
        params['training_params']['cross_folder'] = args.cross_folder
        print('Changed learning rate to: "\033[1m{}\033[0m"'.format(args.cross_folder))
...
def main(parameters, args):
    ...
    if wrapper.is_rank0:
        ...
        if not args.test:
            if parameters.training_params.use_tensorboard:
                ...
                #                 entity='sheercold-14',
                #                 group=log_params.run_name, 
                #                 name=f"{log_params.run_name}/{str(args.cross_folder)}",
                ...
                experiment_name = parameters.training_params.model_name
                current_time = datetime.now().strftime("%Y%m%d-%H%M%S")
                run_dir = f"/data/lishichao/project/Foundation-Medical/wandb/{experiment_name}-{current_time}" 
                try:
                    if args.cross_folder:
                        wandb.init(
                            project=log_params.project_name, 
    ...
    # define trainer 
    trainer = Trainer_save(wrapper)
        
    if parameters.training_params.use_tensorboard:
        trainer.summary_writer = summary_writer
        
    if args.test:
        trainer.test(test_command=True, knn_eval=args.knn_eval)     
    else:
        trainer.train()
